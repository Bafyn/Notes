...
<div id="qwe">
	...
	<form method="post" action="addToBD.php" onSubmit="return CheckSubmitReg()"><br/>
		...
		<input type="text" name="personLogin" onkeyup = "CheckLogin()" id = "log" maxlength="20" placeholder = "Enter login" required pattern = "^[a-zA-Z][a-zA-Z0-9-_\.]{1,20}$"/>
		<p id = "errorLog" name = "errorLogin"></p>

		<input type="email" name="personEmail" onkeyup = "CheckEmail()" id = "email" maxlength="25" placeholder = "Enter E-mail" required pattern = "^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$">
		<p id = "errorEmail" name = "errorEmail"></p>

		<input type="password" name="personPass" onkeyup = "CheckFieldsReg()" id = "pass" maxlength="15" required placeholder = "Enter password"/>
